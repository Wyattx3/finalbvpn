...
// ========== SEED INITIAL DATA ==========
export const seedInitialData = functions.https.onCall(
  async (
    request: functions.https.CallableRequest<{
      adminKey?: string;
    }>
    ...
    // 3. SDUI Configs
    const sduiConfigs: Record<string, object> = {
      home: {
        screen_id: "home",
        config: {
          type: "dashboard",
...
export const getCountryDistribution = functions.https.onCall(async () => {
...
export const getAnalyticsSummary = functions.https.onCall(async () => {
  const now = new Date();
  ...
  // Calculate changes
  const userChange =
    newUsersYesterday.data().count > 0
      ? Math.round(
          ((newUsersToday.data().count - newUsersYesterday.data().count) /
  ...
  const withdrawalChange =
    withdrawalSumLastMonth > 0
      ? Math.round(
          ((withdrawalSumThisMonth - withdrawalSumLastMonth) /
